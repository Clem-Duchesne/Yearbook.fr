...
class Pages_layout_cellule_content extends Base {
  ...
  public function getTextId($cellule, $layout, $pages){
    $sql = "SELECT text_id FROM `{$this->tableName}` WHERE cellule_id = :cellule AND layout_id = :layout AND pages_id = :pages";
    $sth = self::$dbh->prepare($sql);
    $sth->bindValue(':cellule', $cellule);
    $sth->bindValue(':layout', $layout );
    $sth->bindValue(':pages', $pages );
  ...
  public function getImgId($cellule, $layout, $pages){
    $sql = "SELECT image_id FROM `{$this->tableName}` WHERE cellule_id = :cellule AND layout_id = :layout AND pages_id = :pages";
    $sth = self::$dbh->prepare($sql);
    $sth->bindValue(':cellule', $cellule);
    $sth->bindValue(':layout', $layout );
    $sth->bindValue(':pages', $pages );
